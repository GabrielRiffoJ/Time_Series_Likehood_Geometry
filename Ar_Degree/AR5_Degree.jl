...
for i in 1:n
    M[i, i] =a_5^2+a_4^2+a_3^2+a_2^2+a_1^2+1
end

for i in 1:(n-1)
    M[i, i+1] = -a_1+a_1*a_2+a_2*a_3+a_3*a_4+a_4*a_5
    M[i+1, i] = -a_1+a_1*a_2+a_2*a_3+a_3*a_4+a_4*a_5
end

for i in 1:(n-2)
    M[i, i+2] = -a_2+a_1*a_3+a_2*a_4+a_3*a_5
    M[i+2, i] = -a_2+a_1*a_3+a_2*a_4+a_3*a_5
end

for i in 1:(n-3)
    M[i, i+3] = -a_3+a_1*a_4+a_2*a_5
    M[i+3, i] = -a_3+a_1*a_4+a_2*a_5
end
...
ar5[1] = epsilon[1]
ar5[2] = phi[1]*ar5[1] + epsilon[2]
ar5[3] = phi[1]*ar5[2] + phi[2]*ar5[1] + epsilon[3]
ar5[4] = phi[1]*ar5[3] + phi[2]*ar5[2] + phi[3]*ar5[1] + epsilon[4]
ar5[5] = phi[1]*ar5[4] + phi[2]*ar5[3] + phi[3]*ar5[2] + phi[4]*ar5[1] + epsilon[5]

for t in 6:m
    ar5[t] = phi[1]*ar5[t-1] + phi[2]*ar5[t-2] + phi[3]*ar5[t-3] + phi[4]*ar5[t-4] + phi[5]*ar5[t-5] + epsilon[t]
end
...
for solutry in 1:100
    ...
    fa1_str = replace(fa1,"a_1" => "a1","a_2" => "a2","a_3" => "a3","a_4" => "a4","a_5" => "a5")
    fa2_str = replace(fa2,"a_1" => "a1","a_2" => "a2","a_3" => "a3","a_4" => "a4","a_5" => "a5")
    fa3_str = replace(fa3,"a_1" => "a1","a_2" => "a2","a_3" => "a3","a_4" => "a4","a_5" => "a5")
    fa4_str = replace(fa4,"a_1" => "a1","a_2" => "a2","a_3" => "a3","a_4" => "a4","a_5" => "a5")
    fa5_str = replace(fa5,"a_1" => "a1","a_2" => "a2","a_3" => "a3","a_4" => "a4","a_5" => "a5")
    ...
    F = System([fa1_expr,fa2_expr,fa3_expr,fa4_expr,fa5_expr])

    resultar5 = HomotopyContinuation.solve(F)


    cert=HomotopyContinuation.certify(F,resultar5)
    ...
    for i in 1:length(nonsingular(resultar5))

        if is_certified(certificates(cert)[i])
                contcert=contcert+1
                b5=solution(nonsingular(resultar5)[i])[5]
                b4=solution(nonsingular(resultar5)[i])[4]
                b3=solution(nonsingular(resultar5)[i])[3]
                b2=solution(nonsingular(resultar5)[i])[2]
                b1=solution(nonsingular(resultar5)[i])[1]
                expr_sub=abs(substitute(deter, Dict(a_1 => b1,a_2 => b2,a_3 => b3, a_4 => b4,a_5 => b5)))
                if expr_sub <1e-8
                    contne=contne+1
                else 
                    contf=contf+1
    ...
    println("Total: ",nresults(resultar5,only_nonsingular=true)," Cert: ",ndistinct_certified(cert), " Non_stationary: ",contne, " Final solutions: ", contf)
    push!(contsol,nresults(resultar5,only_nonsingular=true))
